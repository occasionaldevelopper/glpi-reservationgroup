...
class Reservation extends CommonDBChild {
   ...
   **/
   function showForm($ID, $options = []) {
      ...
      echo "</td></tr>\n<tr class='tab_bg_2'><td>".__('Group')."</td>";
      $uidg = (empty($ID) ? Session::getLoginUserID() : $resa->fields['groups_id']);
      echo "<td>";
      if (!Session::haveRight("reservation", UPDATE)
          || is_null($item)
          || !Session::haveAccessToEntity($item->fields["entities_id"])) {

         echo "<input type='hidden' name='groups_id' value='".$uidg."'>";
         echo Dropdown::getDropdownGroup(
            Group::getTable(),
            $uidg
         );
      } else {
      ...
      if (empty($ID)) {
        $resa->fields["begin"]=date("Y-m-d 00:00:00",strtotime($resa->fields["begin"]));
        $resa->fields["end"]=date("Y-m-d 23:59:59",strtotime($resa->fields["end"]));
      }
   ...
   **/
   static function displayReservationsForAnItem($ID, $date) {
      ...
      $iterator = $DB->request("SELECT glpi_reservations.id,reservationitems_id, begin, end, users_id, groups_id, comment, groupname FROM glpi_reservations LEFT JOIN (SELECT id, completename as groupname FROM glpi_groups) as groupp ON groupp.id=glpi_reservations.groups_id WHERE end>'" . $debut . "' AND begin<'" . $fin . "' AND reservationitems_id='" . $ID . "' ORDER BY begin");
      ...
      if (count($iterator)) {
         ...
         while ($row = $iterator->next()) {
            ...
            $userdisplay=formatUserName($user->fields["id"], $user->fields["name"], $user->fields["realname"],
                           $user->fields["firstname"]);
            if ($row["groups_id"]!="" AND $row["groups_id"]>0) {
              if ($row["users_id"]=="" OR $row["users_id"]==0) {
                $userdisplay=$row['groupname'];
              } else {
                $userdisplay=$userdisplay . "<br>" . $row['groupname'];
              }
            }
   ...
   **/
   static function showForItem(CommonDBTM $item, $withtemplate = 0) {
      ...
      if ($ri->getFromDBbyItem($item->getType(), $item->getID())) {
         ...
         $iterator = $DB->request("SELECT * FROM glpi_reservations LEFT JOIN (SELECT id, completename as groupname FROM glpi_groups) as groupp ON groupp.id=glpi_reservations.groups_id WHERE end>'" . $now . "' AND reservationitems_id='" . $ri->fields['id'] . "' ORDER BY begin");

         if ($item->getType() == 'Group') {
          if ($item->fields['groups_id']>0) {
              $iterator = $DB->request("SELECT * FROM glpi_reservations LEFT JOIN (SELECT id, completename as groupname FROM glpi_groups) as groupp ON groupp.id=glpi_reservations.groups_id LEFT JOIN glpi_reservationitems ON glpi_reservationitems.id=glpi_reservations.reservationitems_id WHERE end>'" . $now . "' AND groups_id='" . $item->fields['id'] . "' ORDER BY begin");            
          }
         }

         echo "<table class='tab_cadre_fixehov'><tr><th colspan='9'>";
         ...
         if (!count($iterator)) {
            echo "<tr class='tab_bg_2'>";
            echo "<td class='center' colspan='9'>".__('No reservation')."</td></tr>\n";
...
if ($item->getType() == 'Group') {
  $ri2 = new ReservationItem();
            echo "<td class='center'>".str_replace("PluginGenericobject","",$data['itemtype'])."</td>";

            if ($ri2->getFromDB($data["reservationitems_id"])) {
               $link = "&nbsp;";

               if ($item2 = getItemForItemtype($ri2->fields['itemtype'])) {
                  if ($item2->getFromDB($ri2->fields['items_id'])) {
                     $link = $item2->getLink();
                  }
               }
               ...
               echo "</td>";
               echo "<td class='center'>".$data["groupname"]."</td>";
               echo "<td class='center'>".nl2br($data["comment"])."</td>";
               echo "<td class='center'>";
               if (Session::haveRight('reservation', ReservationItem::RESERVEANITEM)) {
                  list($annee, $mois, $jour) = explode("-", $data["begin"]);
                  echo "<a href='".$CFG_GLPI["root_doc"]."/front/reservation.php?reservationitems_id=".
                        $data['reservationitems_id'] ."&amp;mois_courant=$mois&amp;annee_courante=$annee' title=\"".
                        __s('See planning')."\">";
         ...
         $iterator = $DB->request("SELECT * FROM glpi_reservations LEFT JOIN (SELECT id, completename as groupname FROM glpi_groups) as groupp ON groupp.id=glpi_reservations.groups_id WHERE end<='" . $now . "' AND reservationitems_id='" . $ri->fields['id'] . "' ORDER BY begin DESC");
         ...
         echo "<div class='spaced'><table class='tab_cadre_fixehov'><tr><th colspan='9'>";
...
if ($item->getType() == 'Group') {
  ...
  $iterator = $DB->request("SELECT begin, end, items_id,glpi_reservationitems.entities_id,users_id,glpi_reservations.groups_id, groupp.groupname,glpi_reservations.comment,reservationitems_id,completename,itemtype FROM glpi_reservations LEFT JOIN glpi_reservationitems ON glpi_reservationitems.id=glpi_reservations.reservationitems_id LEFT JOIN glpi_entities ON glpi_reservationitems.entities_id=glpi_entities.id LEFT JOIN (SELECT id, completename as groupname FROM glpi_groups) as groupp ON groupp.id=glpi_reservations.groups_id WHERE end>'" . $now . "' AND users_id='" . $ID . "' ORDER BY begin");
      ...
      if (count($iterator) == 0) {
         echo "<tr class='tab_bg_2'>";
         echo "<td class='center' colspan='7'>".__('No reservation')."</td></tr\n>";
  ...
  $iterator = $DB->request("SELECT begin, end, items_id,glpi_reservationitems.entities_id,users_id,glpi_reservations.groups_id, groupp.groupname,glpi_reservations.comment,reservationitems_id,completename,itemtype FROM glpi_reservations LEFT JOIN glpi_reservationitems ON glpi_reservationitems.id=glpi_reservations.reservationitems_id LEFT JOIN glpi_entities ON glpi_reservationitems.entities_id=glpi_entities.id LEFT JOIN (SELECT id, completename as groupname FROM glpi_groups) as groupp ON groupp.id=glpi_reservations.groups_id WHERE end<='" . $now . "' AND users_id='" . $ID . "' ORDER BY begin DESC");
      ...
      echo "<table class='tab_cadre_fixehov'>";
      echo "<tr><th colspan='7'>".__('Past reservations')."</th></tr>\n";
      ...
      if (count($iterator) == 0) {
         ...
         while ($data = $iterator->next()) {
            ...
            $usernamee="";
            if ($data["users_id"]!="" AND $data["users_id"]!="0") {
              $usernamee=getUserName($data["users_id"]);
            }
            echo "<td class='center'>".$usernamee ."</td>";
            echo "<td class='center'>".$data['groupname']."</td>";
            echo "<td class='center'>".nl2br($data["comment"])."</td>";
            echo "<td class='center'>";
            list($annee, $mois, $jour) = explode("-", $data["begin"]);
